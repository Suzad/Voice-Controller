...
/**
 * <p>
 * A Microphone captures audio data from the system's underlying audio input
 * systems. Converts these audio data into Data objects. When the method
 * <code>startRecording()</code> is called, a new thread will be created and
 * used to capture audio, and will stop when <code>stopRecording()</code> is
 * called. Calling <code>getData()</code> returns the captured audio data as
 * Data objects.
 * </p>
 * <p>
 * This Microphone will attempt to obtain an audio device with the format
 * specified in the configuration. If such a device with that format cannot be
 * obtained, it will try to obtain a device with an audio format that has a
 * higher sample rate than the configured sample rate, while the other
 * parameters of the format (i.e., sample size, endianness, sign, and channel)
 * remain the same. If, again, no such device can be obtained, it flags an
 * error, and a call <code>startRecording</code> returns false.
 * </p>
 */
public class Microphone extends BaseDataProcessor {
	...
	/**
	 * The property that specifies whether or not the microphone will release the
	 * audio between utterances. On certain systems (Linux for one), closing and
	 * reopening the audio does not work too well. The default is false for Linux
	 * systems, true for others.
	 */
	@S4Boolean(defaultValue = true)
	public final static String PROP_CLOSE_BETWEEN_UTTERANCES = "closeBetweenUtterances";

	/**
	 * The property that specifies the number of milliseconds of audio data to read
	...
	/**
	 * The property that specifies whether to keep the audio data of an utterance
	 ...
	 * the possible values are "average", which averages the samples from at each
	 * channel, or "selectChannel", which chooses audio only from that channel. If
	 * you choose "selectChannel", you should also specify which channel to use with
	 * the "selectChannel" property.
	 */
	 ...
	 * (which means let the AudioSystem decide), "last," (which means select the
	 * last Mixer supported by the AudioSystem), which appears to be what is often
	 * used for USB headsets, or an integer value which represents the index of the
	 * Mixer.Info that is returned by AudioSystem.getMixerInfo(). To get the list of
	 * Mixer.Info objects, run the AudioTool application with a command line
	 * argument of "-dumpMixers".
	 *
	...
	/**
	 * The property that specifies the size of the buffer used to store audio
	 * samples recorded from the microphone. Default value correspond to 200ms.
	 * Smaller value decrease microphone latency with danger of dropping out the
	 * frames if decoding thread will be slow enough to process the result.
	 */
	 ...
	 * @param closeBetweenUtterances
	 *            whether or not the microphone will release the audio between
	 *            utterances. On certain systems (Linux for one), closing and
	 *            reopening the audio does not work too well. The default is false
	 *            for Linux systems, true for others
	 * @param msecPerRead
	 *            the number of milliseconds of audio data to read each time from
	 *            the underlying Java Sound audio device.
	 * @param keepLastAudio
	 *            whether to keep the audio data of an utterance around until the
	 *            next utterance is recorded.
	 * @param stereoToMono
	 *            how to convert stereo audio to mono. Currently, the possible
	 *            values are "average", which averages the samples from at each
	 *            channel, or "selectChannel", which chooses audio only from that
	 *            channel. If you choose "selectChannel", you should also specify
	 *            which channel to use with the "selectChannel" property.
	 * @param selectedChannel
	 *            the channel to use if the audio is stereo
	 * @param selectedMixerIndex
	 *            the mixer to use. The value can be "default," (which means let the
	 *            AudioSystem decide), "last," (which means select the last Mixer
	 *            supported by the AudioSystem), which appears to be what is often
	 *            used for USB headsets, or an integer value which represents the
	 *            index of the Mixer.Info that is returned by
	 *            AudioSystem.getMixerInfo(). To get the list of Mixer.Info objects,
	 *            run the AudioTool application with a command line argument of
	 *            "-dumpMixers".
	 * @param audioBufferSize
	 *            buffer size
	 */
	public Microphone(int sampleRate, int bitsPerSample, int channels, boolean bigEndian, boolean signed,
			boolean closeBetweenUtterances, int msecPerRead, boolean keepLastAudio, String stereoToMono,
			int selectedChannel, String selectedMixerIndex, int audioBufferSize) {
		initLogger();
	...
	public void newProperties(PropertySheet ps) throws PropertyException {
		...
		desiredFormat = new AudioFormat(sampleRate, sampleSizeInBits, channels, signed, bigEndian);
	...
	public void initialize() {
		...
		/*
		 * If we cannot get an audio line that matches the desired characteristics,
		 * shoot for one that matches almost everything we want, but has a higher sample
		 * rate.
		 */
	...
	private TargetDataLine getAudioLine() {
		...
		try {
			 ...
			 * capabilities of the underlying audio system. The final format will have all
			 * the desired audio characteristics, but may have a sample rate that is higher
			 * than desired. The idea here is that we'll let the processors in the front end
			 * (e.g., the FFT) handle some form of downsampling for us.
			 */
			...
			/*
			 * We either get the audio from the AudioSystem (our default choice), or use a
			 * specific Mixer if the selectedMixerIndex property has been set.
			 */
	 ...
	 * audio. Attempts to create a converter if the requested audio format is not
	 * directly available.
	 *
		...
		if (audioLine != null) {
			if (!audioLine.isOpen()) {
				...
				frameSizeInBytes = (audioStream.getFormat().getSampleSizeInBits() / 8)
						* (int) (sec * audioStream.getFormat().getSampleRate()) * desiredFormat.getChannels();
	...
	/**
	 * Returns the format of the audio recorded by this Microphone. Note that this
	 * might be different from the configured format.
	 *
	...
	/**
	 * Starts recording audio. This method will return only when a START event is
	 * received, meaning that this Microphone has started capturing audio.
	 *
	...
	/**
	 * Stops recording audio. This method does not return until recording has been
	 * stopped and all data has been read from the audio line.
	 */
	...
	class RecordingThread extends Thread {
		...
		/**
		 * Stops the thread. This method does not return until recording has actually
		 * stopped, and all the data has been read from the audio line.
		 */
		public void stopRecording() {
			...
			// flush can not be called here because the audio-line might has been set to
			// null already by the mic-thread
			// audioLine.flush();
		}

		/**
		 * Implements the run() method of the Thread class. Records audio, and cache
		 * them in the audio buffer.
		 */
		@Override
		public void run() {
			totalSamplesRead = 0;
			...
			try {
				...
				if (closeBetweenUtterances) {
					/*
					 * Closing the audio stream *should* (we think) also close the audio line, but
					 * it doesn't appear to do this on the Mac. In addition, once the audio line is
					 * closed, re-opening it on the Mac causes some issues. The Java sound spec is
					 * also kind of ambiguous about whether a closed line can be re-opened.
					 * So...we'll go for the conservative route and never attempt to re-open a
					 * closed line.
					 */
					audioStream.close();
					audioLine.close();
			...
			long duration = (long) (((double) totalSamplesRead / (double) audioStream.getFormat().getSampleRate())
					* 1000.0);
	 ...
	 * @param channels
	 *            the number of channels in the stereo audio
	 */
	...
	/**
	 * Reads and returns the next Data object from this Microphone, return null if
	 * there is no more audio data. All audio data captured in-between
	 * <code>startRecording()</code> and <code>stopRecording()</code> is cached in
	 * an Utterance object. Calling this method basically returns the next chunk of
	 * audio data cached in this Utterance.
	 *
	...
	/**
	 * Returns true if there is more data in the Microphone. This happens either if
	 * the a DataEndSignal data was not taken from the buffer, or if the buffer in
	 * the Microphone is not yet empty.
	 *
	...
	public synchronized void interruptRecording() {
		clear();
		audioList.add(new DataEndSignal(0));
	}
